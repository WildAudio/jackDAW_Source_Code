...
namespace juce
{
	...
	@see Reverb
	*/
	class JUCE_API  CustomAudioTransportSource : public AudioSource
	{
	public:
		/** Creates a CustomAudioTransportSource to process a given input source.

		@param inputSource              the input source to read from - this must not be null
		@param deleteInputWhenDeleted   if true, the input source will be deleted when
		this object is deleted
		*/
		CustomAudioTransportSource(AudioSource* inputSource,
			bool deleteInputWhenDeleted);

		/** Destructor. */
		~CustomAudioTransportSource();
		...
		void setPanL(float panLToUse);
		void setPanR(float panRToUse);
	...
	private:
		...
		JUCE_DECLARE_NON_COPYABLE_WITH_LEAK_DETECTOR(CustomAudioTransportSource)
	};
